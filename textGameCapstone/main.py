...
COMMAND_MESSAGES = {
    'SUCCESS': f"{GREEN}You've retrieved the {{}} and added it to your inventory!{RESET}",
    'ALREADY_HAVE': f"{YELLOW}You already have the {{}}{RESET}!",
    'WRONG_ITEM': f"{YELLOW}There is an item you need in here, its not {{}}...{RESET}\nNOTE: All items will be a single word.",
    'NO_ITEM': f"{YELLOW}There is no item in this room for you to pick up.{RESET}",
    'INVALID_MOVE': f"{RED}There isn't a door to the {{}} from here. Please choose a valid direction.{RESET}",
    'UNRECOGNIZED': f"{RED}Sorry the command you entered was unrecognized. Type 'help' for commands.{RESET}",
    'WINNER': f"""~""" * 90 + f"""
As you breach the {CYAN}Arcane Dungeon{RESET}, a wave of stifling heat and flame washes over you.
Your {MAGENTA}magical robes{RESET} flare with protective wards, turning the inferno aside as you press
forward. Looming in the shadows, five pairs of menacing eyes—glowing green, red, blue,
purple, and white—fix upon you. The light of your {MAGENTA}staff{RESET} pierces the gloom, revealing the 
...
raise your {MAGENTA}shield{RESET}, bracing as the raw energy slams into it. The impact vibrates up your arm,
...
Suddenly, jagged spears of ice rain from the gaping maw of the blue head. You invoke your {MAGENTA}cloak of
levitation{RESET}, soaring above the deadly volley. As you rise, you come face-to-face with the green hydra
...
devastating hyperbeam. With the last of your strength, you raise to your feet and slam the butt of your {MAGENTA}staff 
{RESET}into the ground, erecting a shimmering wall of crystal. The immense energy released from the hydra lances past,
shaking the dungeon to its foundations. In these fleeting, precious moments, you consume the restorative {MAGENTA}stew{RESET}
and potent {MAGENTA}mana potion{RESET}, feeling vitality and magic surge back into you. The hyperbeam sends cracks through your
crystal defense as the other heads join, blasting the wall with fire, frost, lightning, and acid.

Fractures spiderweb across the crystal. You channel your will into the crown of your {MAGENTA}staff{RESET}, its gems whirling into a
...
your {MAGENTA}staff{RESET}... and unleash it all with a final, deafening blast. As the dust settles, the hydra lies collapsed, its
...
{GREEN}CONGRATULATIONS! You have saved the city and won the game!{RESET}
""" + """~""" * 90,
...
class MainGame:
    ...
    def _display_game_state(self, message=None):
        ...
        print(f"You are in the {CYAN}{self.player.current_room.name}{RESET}.")
        ...
        if num_exits == 1:
            # Case 1: One exit
            print(f"\nYou see a door to the: {CYAN}{exit_list[0]}{RESET}")
        elif num_exits == 2:
            # Case 2: Two exits
            print(f"\nYou see doors to the: {CYAN}{exit_list[0]} and {exit_list[1]}{RESET}")
        elif num_exits > 2:
            ...
            print(f"\nYou see doors to the: {CYAN}{all_but_last}, and {last_exit}{RESET}")
    ...
    def _process_input(self, user_input):
        ...
        if command == 'GO':
            ...
            if direction not in DIRECTIONS:
                ...
                self._display_message(f"{RED}'{direction}' is not a valid direction. Try NORTH, SOUTH, EAST, or WEST.{RESET}")
                return

            # The Player object attempts the move, using the GameMap for lookups
            self._display_message(f"{GREEN}You go {direction}{RESET}.")
    ...
    def save_game(self):
        ...
        if self.db_manager.save_game_state(self.player, self.game_map):
            self._display_message(f"{GREEN}Game Saved Successfully!{RESET}")
        else:
            self._display_message(f"{RED}Error saving game.{RESET}")
    ...
    def load_game(self):
        ...
        if save_data:
            ...
            self._display_message(f"{GREEN}Game Loaded Successfully!{RESET}")
            return True

        self._display_message(f"{RED}No saved game found.{RESET}")
        return False
    ...
    def start_game(self):
        ...
        print("|                                                                                     |")
        print(f"|{YELLOW} NOTE: All items will be a single word.{GREEN}                                              |")
        print(f"| {RED}WARNING: Do not try to face the hydra until you have gathered all your supplies!{GREEN}    |")
        print(f"~" * 87)
    ...
    def run_game(self):
        ...
        while self.is_running:
            try:
                # Get user input
                user_input_raw = input(f"{BLUE}What would you like to do?{RESET}\n> ").strip()
